...
const ApplyApartmentModal = ({ isOpen, closeModal, apartment }) => {
  ...
  const nextStep = () => {
    ...
    if (step === 1 && !moveInDate) {
      setErrors({...errors, moveInDate: "Please select a move-in date"});
      return;
    }
    if (step === 1 && (!duration || duration < 1)) {
      setErrors({...errors, duration: "Please enter a valid duration"});
      return;
    }
    setStep(step + 1);
  };
